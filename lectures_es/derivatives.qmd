...
ax.text(S_mean - 1, payoff_mean, "E[call_payoff(S)]", 
                          ...
                          fontsize=10, 
                          color='blue')
ax.text(S_mean + 0.7, 1.6, "{", 
                          ha='right', 
                          fontsize=92, 
                          color='grey', 
                          fontweight='ultralight', 
                          fontname="serif")
ax.text(S_mean - 3, 2.5, "Intrinsic value", 
                          ha='right', 
                          fontsize=10)
...
ax.text(105.7, 1.5 , "{", ha='right', fontsize=92, 
                      color='grey', 
                      fontweight='ultralight', 
                      fontname="serif")

ax.text(97, 2.4 , "Intrinsic value", ha='right', 
                                        fontsize=10)
ax.text(102, payoff_mean+3 , "$C_t$", ha='left', 
                                        fontsize=12)
ax.text(102, 4 , "$C_T$", ha='left', fontsize=12)
...
In the original papers, the derivation of the Black and Scholes option pricing formula, uses som pretty fancy math and sophisticated theory. However, if we know that we can price any derivative using risk neutral expectations, it becomes quite easy to price any derivative. 
...
## Nummerical approximation

If you want to price your own derivative, you can take inspiration from this nummerical calcualation of the integral above:
...
def integrand(epsilon):
    ...
    payoff = np.maximum(ST - X, 0)  # Call option payoff
    pdf = stats.norm.pdf(epsilon)  # Standard normal density function
    return payoff * pdf

# Compute the integral numerically
integral_value, error = integrate.quad(integrand, -np.inf, np.inf)

# Compute the discounted expectation
f0_numerical = np.exp(-r * T) * integral_value

# Compute the Black-Scholes closed-form solution for comparison
d1 = (np.log(S0 / X) + (r + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
d2 = d1 - sigma * np.sqrt(T)
bs_price = S0 * stats.norm.cdf(d1) - X * np.exp(-r * T) * stats.norm.cdf(d2)

# Display results
f0_numerical, bs_price, abs(f0_numerical - bs_price)
